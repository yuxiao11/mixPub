 ...
 */
public class VideoCRecall implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(VideoCRecall.class);
    ...
    public VideoCRecall(MixRequestInfo mixRequestInfo, List<RecordInfo> recordInfoList, TagPeriod tagPeriod, int number) {
    ...
    public List<RecallResult> call() throws Exception {
        ...
        try {
            ...
            logger.error("VideoCRecall recall executor:{}", e.toString());
            return Collections.emptyList();
        }
    }
...
//        logger.info("step222:{}",mixRequestInfo.getUid());
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod) {
        WhyReason whyReasonTrue = WhyReason.VIDEO_C_LONG;

...
public class DocpicScRecall implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(DocpicScRecall.class);
    ...
    public DocpicScRecall(MixRequestInfo mixRequestInfo, List<RecordInfo> recordInfoList, TagPeriod tagPeriod, int number) {
    ...
    public List<RecallResult> call() throws Exception {
        ...
        try {
            ...
            logger.error("DocpicsCRecall recall executor: {}", e.toString());
            return Collections.emptyList();
        }
    }
    ...
    private List<RecallResult> doRecall() {
         ...
         * 拼redis key |pin tag
         */
        Set<String> newRecallTagSet = recallTagSet.stream().map(s -> GyConstant.subcate_prefix + s).collect(Collectors.toSet());
        if ("3619988e26104ce99bb230d2d0335a91".equals(mixRequestInfo.getUid())) {
            logger.info("@@DocpicC@@ step2.5 uid:{}", mixRequestInfo.getUid());
        }
        //tag 和 id map, id List 按hotboost 有序
        Map<String, List<String>> tagIdsImmutable = CotagDocsNewCache.getFromCache(newRecallTagSet);
        if ("3619988e26104ce99bb230d2d0335a91".equals(mixRequestInfo.getUid())) {
            logger.info("@@DocpicC@@ step3 uid:{}", mixRequestInfo.getUid());
        }
          /*
           还原tag
         */
        ...
        while (iterator.hasNext()) {
            ...
            String key = next.getKey().substring(GyConstant.subcate_prefix.length());
            List<String> value = next.getValue();
            tagIds.put(key, value);
        }
        if ("3619988e26104ce99bb230d2d0335a91".equals(mixRequestInfo.getUid())) {
            logger.info("@@DocpicC@@ step4 uid:{}, tagIds:{}", mixRequestInfo.getUid(), tagIds);
        }
        //id 和 tag map
        ...
        if ("3619988e26104ce99bb230d2d0335a91".equals(mixRequestInfo.getUid())) {
            logger.info("@@DocpicC@@ step5 uid:{}, resultDocs:{}", mixRequestInfo.getUid(), resultDocs);
        }
        ...
        if ("3619988e26104ce99bb230d2d0335a91".equals(mixRequestInfo.getUid())) {
            logger.info("@@DocpicC@@ step6 uid:{}, resultDocs:{}", mixRequestInfo.getUid(), resultDocs);
        }
        return resultDocs;
    }
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod) {
        WhyReason whyReasonTrue = WhyReason.DOCPIC_SC_LONG;

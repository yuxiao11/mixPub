 ...
 */
public class VideoScRecall implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(VideoScRecall.class);
    ...
    public VideoScRecall(MixRequestInfo mixRequestInfo, List<RecordInfo> recordInfoList, TagPeriod tagPeriod, int number) {
    ...
    public List<RecallResult> call() throws Exception {
        ...
        try {
            ...
            logger.error("VideoScRecall recall executor:{}", e.toString());
            return Collections.emptyList();
        }
    }
    ...
    private List<RecallResult> doRecall() {
        ...
        Map<String, List<String>> tagIdsImmutable = CotagVideoNewCache.getFromCache(newRecallTagSet);
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod) {
        WhyReason whyReasonTrue = WhyReason.VIDEO_SC_LONG;

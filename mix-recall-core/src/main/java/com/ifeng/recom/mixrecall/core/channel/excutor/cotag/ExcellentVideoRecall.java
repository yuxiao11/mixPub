 ...
 */
public class ExcellentVideoRecall implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(ExcellentVideoRecall.class);
    ...
    public ExcellentVideoRecall(MixRequestInfo mixRequestInfo, List<RecordInfo> recordInfoList, TagPeriod tagPeriod, int number) {
    ...
    public List<RecallResult> call() throws Exception {
        ...
        try {
            ...
            logger.error("ExcellentVideoRecall recall executor", e);
            return Collections.emptyList();
        }
    }
    ...
    private List<RecallResult> doRecall() {
         ...
         * 拼redis key |pin tag
         */
        Set<String> newRecallTagSet= recallTagSet.stream().map(s-> GyConstant.video_prefix + s).collect(Collectors.toSet());
        //tag 和 id map, id List 按hotboost 有序
        Map<String, List<String>> tagIdsImmutable = ExcellentVideoCache.getFromCache(newRecallTagSet);
        ...
        while (iterator.hasNext()){
            ...
            String key = next.getKey().substring(GyConstant.video_prefix.length());
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod) {
        ...
        if (tagPeriod.equals(TagPeriod.LONG)) {
            whyReasonTrue = WhyReason.EXCELLENT_V_C;
        }else if(tagPeriod.equals(TagPeriod.RECENT)){
            whyReasonTrue = WhyReason.EXCELLENT_V_SC;
        }

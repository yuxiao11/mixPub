...
public class UserCFRecallExecutorALS implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(UserCFRecallExecutorALS.class);
    ...
    private static final Integer NEIGHBOR_CLICK_NUM = 1500;//YX 更改召回数量 1500 -> 1000
    private static final Integer USERCF_RECALL_NUM = 800;//YX 这里是最后召回的数量 800 -> 500
    ...
    public UserCFRecallExecutorALS(MixRequestInfo mixRequestInfo, UserCF userCF) {
    ...
    public List<RecallResult> doRecall() {
        ...
        timer.addStartTime("ucf_a");

        timer.addStartTime("ucf_a_cli");
        ...
        timer.addEndTime("ucf_a_cli");


        timer.addStartTime("ucf_a_st");
        Map<String, DocCtrInfo> docId2Stat = getDocId2Stat(clickIds);
        timer.addEndTime("ucf_a_st");

        timer.addStartTime("ucf_a_rank");
        ...
        timer.addEndTime("ucf_a");

        TimerEntity.TimeBean ucf = timer.getTimeBean("ucf_a");

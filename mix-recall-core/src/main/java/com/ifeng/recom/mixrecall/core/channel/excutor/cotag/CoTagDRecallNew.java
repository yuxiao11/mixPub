...
import static com.ifeng.recom.mixrecall.common.util.UserUtils.getABtestTag;
import static com.ifeng.recom.mixrecall.core.util.RecallUtils.*;

/**
 * Created by geyl on 2017/11/13.
 */
public class CoTagDRecallNew implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(CoTagDRecallNew.class);
    ...
    public CoTagDRecallNew(MixRequestInfo mixRequestInfo, List<RecordInfo> recordInfoList, TagPeriod tagPeriod, int number) {
    ...
    public List<RecallResult> call() throws Exception {
        ...
        try {
            ...
            logger.error("{} cotagNew recall executor ",mixRequestInfo.getUid(), e);
            return Collections.emptyList();
        }
    }
    ...
    private List<RecallResult> doRecall() {
        ...
        Map<String, List<String>> tagIdsImmutable = CotagDocsNewCache.getFromCache(recallTagSet);
        ...
        if (MathUtil.getNum(100) == 1) {
            logger.debug("uid:{} num:{}", mixRequestInfo.getUid(), GsonUtil.object2jsonWithoutExpose(tagRecallNum, Map.class));
        }
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod,MixRequestInfo mixRequestInfo) {
        ...
        if (tagPeriod.equals(TagPeriod.LONG)) {
            ...
            whyReasonTrue = WhyReason.COTAG_D_RECENT_n;
        } else if (tagPeriod.equals(TagPeriod.LAST)) {
            whyReasonTrue = WhyReason.COTAG_D_LAST_n;
        }
        for (RecallResult recallResult : recallResults) {

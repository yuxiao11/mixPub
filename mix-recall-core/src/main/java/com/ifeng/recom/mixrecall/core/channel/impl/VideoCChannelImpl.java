...
import com.ifeng.recom.mixrecall.core.channel.excutor.cotag.ExcellentVideoRecall;
import com.ifeng.recom.mixrecall.core.channel.excutor.csc.VideoCRecall;
...
@Service
public class VideoCChannelImpl {
    private static final Logger logger = LoggerFactory.getLogger(VideoCChannelImpl.class);
    ...
    public List<RecallResult> doRecall(MixRequestInfo mixRequestInfo) {
        ...
        List<RecallResult> result = callVideoC(mixRequestInfo, logicParams, userModel);

        long cost = System.currentTimeMillis() - startTime;
        if (cost > 50) {
            ServiceLogUtil.debug("VideoCChannel {} cost:{}", uid, cost);
        }

        return result;
    }

    private List<RecallResult> callVideoC(MixRequestInfo mixRequestInfo, LogicParams logicParams, UserModel userModel) {
        List<RecallResult> recallResults = new ArrayList<>();
        Map<String, Future<List<RecallResult>>> threadResultMap = new HashMap<>();
        Future<List<RecallResult>> videoCResult = null;

        try {
            videoCResult = ExecutorThreadPool.submitExecutorTask(new VideoCRecall(mixRequestInfo, UserProfileUtils.profileTagWeightFilter(userModel.getVideo_cate(),RecallConstant.PROFILE_CUT_OFF_WEIGHT), TagPeriod.LONG, logicParams.videoCNum));
            if (videoCResult != null) {
                threadResultMap.put(TagPeriod.LONG.toString(), videoCResult);
            }

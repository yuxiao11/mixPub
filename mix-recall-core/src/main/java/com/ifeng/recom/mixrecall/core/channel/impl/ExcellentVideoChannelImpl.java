...
@Service
public class ExcellentVideoChannelImpl {
    private static final Logger logger = LoggerFactory.getLogger(ExcellentVideoChannelImpl.class);
    ...
    public List<RecallResult> doRecall(MixRequestInfo mixRequestInfo) {
        ...
        if (cost > 50) {
            ServiceLogUtil.debug("ExcellentVideoChannelImpl {} cost:{}", uid, cost);
        }
    ...
    private List<RecallResult> callExcellent(MixRequestInfo mixRequestInfo, LogicParams logicParams, UserModel userModel) {
        ...
        try {
            excellentCResult = ExecutorThreadPool.submitExecutorTask(new ExcellentVideoRecall(mixRequestInfo, userModel.getVideo_cate(), TagPeriod.LONG, logicParams.getVideoExcellentCNum()));
            excellentScResult = ExecutorThreadPool.submitExecutorTask(new ExcellentVideoRecall(mixRequestInfo,  UserProfileUtils.profileTagWeightFilter(userModel.getVideo_subcate(),RecallConstant.PROFILE_CUT_OFF_WEIGHT), TagPeriod.RECENT, logicParams.getVideoExcellentScNum()));
            ...
            e.printStackTrace();
            logger.error("{} ExcellentVideoChannelImpl ERROR:{}", mixRequestInfo.getUid(), e.toString());

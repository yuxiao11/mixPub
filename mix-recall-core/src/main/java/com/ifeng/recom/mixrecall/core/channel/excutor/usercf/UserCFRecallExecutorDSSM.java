...
public class UserCFRecallExecutorDSSM implements Callable<List<RecallResult>> {
    private static final Logger logger = LoggerFactory.getLogger(UserCFRecallExecutorDSSM.class);
    ...
    private static final Integer NEIGHBOR_CLICK_NUM = 1500;
    private static final Integer USERCF_RECALL_NUM = 1000;//YX
    ...
    public UserCFRecallExecutorDSSM(MixRequestInfo mixRequestInfo, UserCF userCF) {
    ...
    public List<RecallResult> doRecall() {
        ...
        timer.addStartTime("ucf_d");

        timer.addStartTime("ucf_d_cli");
        ...
        timer.addEndTime("ucf_d_cli");


        timer.addStartTime("ucf_d_st");
        Map<String, DocCtrInfo> docId2Stat = getDocId2Stat(clickIds);
        timer.addEndTime("ucf_d_st");

        timer.addStartTime("ucf_d_rank");
        ...
        timer.addEndTime("ucf_d");

        TimerEntity.TimeBean ucf=timer.getTimeBean("ucf_d");
        if (ucf != null && ucf.getUsedTime() > 400) {
            timeLogger.info("ChannelLog ucf_d(dssm) {},loopTime:{},baseDocSize:{}, uid:{}", timer.getStaticsInfo(), loopTime, baseDocSize, mixRequestInfo.getUid());
        }

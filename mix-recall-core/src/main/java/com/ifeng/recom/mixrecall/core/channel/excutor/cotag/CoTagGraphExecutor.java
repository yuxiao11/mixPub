 ...
 */
public class CoTagGraphExecutor implements Callable<List<RecallResult>> {

    private static final Logger logger = LoggerFactory.getLogger(CoTagGraphExecutor.class);
    ...
    public CoTagGraphExecutor(MixRequestInfo mixRequestInfo, List<String> recordInfoList, DocType docType, TagPeriod tagPeriod, int number){
    ...
    private List<RecallResult> doRecall() {
        ...
        //计算每个标签应召回的文章数量
        int everyNum=Math.min(Math.max(number/recordInfoList.size(),5),15);
        int tagNum=number/everyNum;
        recordInfoList=recordInfoList.subList(0,Math.min(tagNum,recordInfoList.size()));

        tagRecallNum = getTagRecallNumber4Graph(everyNum,recordInfoList);

        logger.info("check cotagGraph uid:{} recordInfo:{}",mixRequestInfo.getUid(),recordInfoList.toString());
        ...
        Map<String,Integer> tagIdsNumber = new HashMap<>();
        Map<String,Integer> tagIdsNumberFiltered = new HashMap<>();
        tagIdsImmutable.forEach((key,value) -> {tagIdsNumber.put(key,value.size());});
        filteredTagDocs.forEach((key,value) -> {tagIdsNumberFiltered.put(key,value.size());});
        logger.info("Check CotagGraph_{} uid:{} recallNum:{} tagNumber:{} tagNumberFiltered:{} finalSize:{}",docType,mixRequestInfo.getUid(),tagRecallNum.toString(),
                tagIdsNumber.toString(),tagIdsNumberFiltered.toString(),resultDocs.size());
    ...
    private static void setWhyInfo(List<RecallResult> recallResults, TagPeriod tagPeriod,DocType docType) {
        ...
        if (tagPeriod.equals(TagPeriod.RECENT)) {
            if(docType.equals(DocType.VIDEO)){
                whyReasonTrue =   WhyReason.COTAG_V_RECENT_GRAPH;
            }else {
                whyReasonTrue = WhyReason.COTAG_D_RECENT_GRAPH;
            }
        } else if (tagPeriod.equals(TagPeriod.LONG)) {
            if(docType.equals(DocType.VIDEO)){
                whyReasonTrue = WhyReason.COTAG_V_LONG_GRAPH;
            }else {
                whyReasonTrue = WhyReason.COTAG_D_LONG_GRAPH;
            }
        }
        for (RecallResult recallResult : recallResults) {

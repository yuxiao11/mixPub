...
import com.ifeng.recom.mixrecall.common.util.bloomfilter.common.IBloomFilter;
import com.ifeng.recom.mixrecall.common.util.bloomfilter.common.IBloomFilterWriter;
 ...
 */
public class ByteBloomFilter implements IBloomFilter, IBloomFilterWriter {
	 ...
	 */
	public ByteBloomFilter(DataInput meta) throws IOException,
	 ...
	 *         number of keys and provide the given error rate, assuming that
	 *         the optimal number of hash functions is used and it does not have
	 *         to be an integer.
	 */
	public static long computeBitSize(long maxKeys, double errorRate) {
		return (long) Math
				.ceil(maxKeys * (-Math.log(errorRate) / LOG2_SQUARED));
	}
	 ...
	 *         to maintain the target error rate, if the number of hash
	 ...
	 * of bits, and number of hash functions. Taken directly from the <a href=
	...
	/**
	 * Determines & initializes bloom filter meta data from user develop.config. Call
	 ...
	 * @param foldFactor
	 *            When finished adding entries, you may be able to 'fold' this
	 *            bloom to save space. Tradeoff potentially excess bytes in
	 ...
	 * 
	 * @param byteSizeHint
	 *            the desired number of bytes for the Bloom filter bit array.
	 *            Will be increased so that folding is possible.
	 * @param errorRate
	...
	void sanityCheck() throws IllegalArgumentException {
		if (0 >= this.byteSize || this.byteSize > Integer.MAX_VALUE) {
			throw new IllegalArgumentException("Invalid byteSize: "
					+ this.byteSize);
		}
	...
	public void add(byte[] buf, ByteBuffer theBloom) {
		// if (theBloom == null) {
		// // In a version 1 HFile Bloom filter data is stored in a separate
		// meta
		// // block which is loaded on demand, but in version 2 it is
		// pre-loaded.
	...
	public static boolean contains(byte[] buf, int offset, int length,
		...
		if (randomGeneratorForTest == null) {
			...
			// Test mode with "fake lookups" to estimate
	...
	/**
	 * {@inheritDoc} Just concatenate row and column by default. May return the
	 * original row buffer if the column qualifier is empty.
	 */
	@Override
...
//		return formatStats(this) + STATS_RECORD_SEP + "Actual error rate: " + String.format("%.8f", actualErrorRate());
		return STATS_RECORD_SEP + "Actual error rate: " + String.format("%.8f", actualErrorRate());
	}
